...
get_variables_from_topic(RedisClient, ScriptData,
                         Topic) ->
    {ok, Result} = eredis:q(RedisClient,
                            ["EVAL", ScriptData, 1, Topic]),
    Result.

get_values_variables(RedisClient, ScriptData,
                     VariablesData) ->
...
get_values_from_topic(Topic, Env) ->
    ...
    UbidotsScriptData =
        get_lua_script_data_from_env_result(get_lua_script_from_base64(proplists:get_value(ubidots_cache_get_values_variables_script_base64,
                                            ...
                                            UbidotsScriptFilePath),
    VariablesData =
        get_variables_from_topic(ReactorRedisClient,
                                 ReactorScriptData,
                                 Topic),
    Values = get_values_variables(UbidotsRedisClient,
                                  UbidotsScriptData,
                                  VariablesData),
    Values.
...
retainer_multiple_lv_test_() ->
    ...
    initialize_mqtt_cache(ReactorRedisClient,
                          ...
                          Devices),
    Result =
        get_values_from_topic("/v1.6/users/token/devices/d1/+/lv",
                              Env),
    eredis:q(ReactorRedisClient, ["FLUSHDB"]),
    eredis:q(UbidotsRedisClient, ["FLUSHDB"]),
    [?_assertEqual([<<"/v1.6/devices/d1/v1/lv">>,
                    <<"11.1">>,
                    <<"/v1.6/devices/d1/v2/lv">>,
                    <<"11.1">>,
...
retainer_multiple_test_() ->
    ...
    initialize_mqtt_cache(ReactorRedisClient,
                          ...
                          Devices),
    Result =
        get_values_from_topic("/v1.6/users/token/devices/d1/+",
                              Env),
    eredis:q(ReactorRedisClient, ["FLUSHDB"]),
    eredis:q(UbidotsRedisClient, ["FLUSHDB"]),
...
retainer_lv_test_() ->
    ...
    initialize_mqtt_cache(ReactorRedisClient,
                          ...
                          Devices),
    [Topic, Value | _Rest] =
        get_values_from_topic("/v1.6/users/token/devices/d1/v1/lv",
                              Env),
    eredis:q(ReactorRedisClient, ["FLUSHDB"]),
    eredis:q(UbidotsRedisClient, ["FLUSHDB"]),
    [?_assertEqual(<<"/v1.6/devices/d1/v1/lv">>, Topic),
     ?_assertEqual(<<"11.1">>, Value)].
...
retainer_test_() ->
    ...
    initialize_mqtt_cache(ReactorRedisClient,
                          ...
                          Devices),
    [Topic, Value | _Rest] =
        get_values_from_topic("/v1.6/users/token/devices/d1/v1",
                              Env),
    eredis:q(ReactorRedisClient, ["FLUSHDB"]),
    eredis:q(UbidotsRedisClient, ["FLUSHDB"]),
    [?_assertEqual(<<"/v1.6/devices/d1/v1">>, Topic),
     ?_assertEqual(<<"{\"value\": 11.1, \"timestamp\": 11, "
                     "\"context\": {\"a\": 11}, \"created_at\": "
                     "11}">>,
                   Value)].
